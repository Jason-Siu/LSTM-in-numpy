...
# regex used to split data
words = list(set(re.split("(\d|\w+|\+=|-=|\*=|\/=|\/\/|==|\s|\[|\]|\(|\)|!|\"|\*|\{|\}|\/|\\|=|:|\+|;|\.|-|~|<|>|\,|\?|@|#|$|%|^|&&|&|\|\||_)", data)))
data = list(re.split("(\d|\w+|\+=|-=|\*=|\/=|\/\/|==|\s|\[|\]|\(|\)|!|\"|\*|\{|\}|\/|\\|=|:|\+|;|\.|-|~|<|>|\,|\?|@|#|$|%|^|&&|&|\|\||_)", data))
vocab_size = len(words)
# dictionaries that convert from word to index and vice versa
char_to_idx = {ch:i for i,ch in enumerate(words)}
idx_to_char = {i:ch for i,ch in enumerate(words)}
